...
import { NEST_APP_OPTIONS, configureApp } from './app-config';
...
async function bootstrap(): Promise<void> {
  ...
  // Security middleware, body parsing, prefix, validation and the exception
  // filter — shared verbatim with the e2e suites so tests exercise the same
  // HTTP layer that ships.
  configureApp(app);
